...
// until released at cleanup), so the eager token mint hangs until its context
// deadline fires. Used to prove the IAM auth call is time-bounded.
...
func fakeADCWithServer(t *testing.T, block bool) {
	...
	// waits for in-flight requests) doesn't hang on the parked goroutine.
...
func TestBuildIAMAuthOption_Disabled(t *testing.T) {
	opt, err := buildIAMAuthOption(context.Background(), configuration.IAMAuthConfig{Enabled: false})
...
func TestBuildIAMAuthOption_Success(t *testing.T) {
	...
	opt, err := buildIAMAuthOption(context.Background(), configuration.IAMAuthConfig{
		Enabled:  true,
		Provider: "gcp",
	})
	require.NoError(t, err)
...
func TestBuildIAMAuthOption_EmptyProvider(t *testing.T) {
	opt, err := buildIAMAuthOption(context.Background(), configuration.IAMAuthConfig{Enabled: true})
	require.Error(t, err)
	assert.Nil(t, opt)
}
...
// With IAM auth enabled, Connect must build the IAM option and establish the KV
// store. The connector is lazy (no dial on construction), so this exercises the
// full IAM success branch offline.
func TestRedisBackend_Connect_IAMEnabled_Success(t *testing.T) {
	fakeADC(t)

	r := &RedisBackend{
		config: &RedisConfig{
			Addrs:   []string{"localhost:6379"},
			IAMAuth: configuration.IAMAuthConfig{Enabled: true, Provider: "gcp"},
		},
	}
	...
	require.NoError(t, err)
	assert.NotNil(t, r.kv, "the KV store must be initialised on a successful IAM connect")
}

// With IAM auth enabled but an unsupported provider, Connect must fail up front,
// before any KV store is built.
func TestRedisBackend_Connect_IAMUnsupportedProvider_Errors(t *testing.T) {
	r := &RedisBackend{
		config: &RedisConfig{
			Addrs:   []string{"localhost:6379"},
			IAMAuth: configuration.IAMAuthConfig{Enabled: true, Provider: "aws"},
		},
	}
